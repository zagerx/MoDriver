...
enum feedback_error_code feedback_init(struct feedback *feedback)
{
	...
	if (param->wheel_radius <= 0.0f || param->gear_ratio <= 0.0f || 
	    param->pole_pairs <= 0.0f ||
...
/* 计算累积机械角度，处理编码器溢出 */
static void feedback_calc_accumulated_mangle(struct feedback *feedback, 
                                              int32_t adjusted_raw)
{
	...
	data->accumulated_mangle_rad = (two_pi / (float)cpr) * 
	                                (float)data->total_counts * param->direction;
...
/* 差分法计算机械角速度，带低通滤波 */
static float feedback_calc_velocity(struct feedback *feedback, float dt, 
                                     float cur_mangle)
{
	struct feedback_data *data = &feedback->data;
