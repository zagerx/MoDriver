...
void fault_handler(struct cia402_instance *inst)
{
	...
	struct statemachine *sm = &inst->pds_sm;
...
void cia402_pds_not_ready_state(struct statemachine *sm)
{
	...
	case CHECK_READY:
		...
		if (inst && inst->is_initialized) {
			sm_transition(sm, cia402_pds_switch_on_disabled_state);
		}
		break;
	...
	case CHECK_COMMAND:
		...
		if (inst && inst->controlword) {
			...
			if (cmd == CMD_SHUTDOWN_REQ) {
				...
				sm_transition(sm, cia402_pds_ready_to_switch_on_state);
			}
		}
		break;
	...
	case CHECK_COMMAND:
		...
		if (inst && inst->controlword) {
			...
			case CMD_SWITCH_ON_REQ:
				...
				sm_transition(sm, cia402_pds_switched_on_state);
				break;
	...
	case CHECK_COMMAND:
		...
		if (inst && inst->controlword) {
			...
			case CMD_ENABLE_OP_REQ:
				...
				sm_transition(sm, cia402_pds_operation_enabled_state);
				break;
 ...
 *
 * PP模式新目标点处理 (bit4):
 * - 检测控制字 bit4 (New set-point) 的上升沿 (0->1)
 * - 调用 motor_set_target_pos 设置目标位置和速度
 * - 设置状态字 bit12 (Set-point acknowledge) = 1 表示已接收
 * - 当 bit4 变为 0 时，清除 bit12
 */
	...
	case RUNING:
		...
		if (inst->target_position && inst->modes_of_operation &&
		    *inst->modes_of_operation == CIA402_MODE_PROFILE_POSITION) {
			...
			/* 当 bit4 变为 0 时，清除 Set-point acknowledge */
			if (!new_set_point_now && new_set_point_prev) {
				*inst->statusword &= ~(1U << 12);
			}
		}
		...
		case CMD_QUICK_STOP_REQ:
			...
			sm_transition(sm, cia402_pds_quick_stop_active_state);
			break;
...
void cia402_pds_fault_reaction_state(struct statemachine *sm)
{
	...
	case CHECK_COMPLETE:
		...
		sm_transition(sm, cia402_pds_fault_state);
		break;
