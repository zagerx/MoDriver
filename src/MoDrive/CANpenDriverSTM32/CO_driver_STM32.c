...
static uint8_t prv_send_can_message(CO_CANmodule_t *CANmodule, CO_CANtx_t *buffer)
{
	...
	if (HAL_FDCAN_GetTxFifoFreeLevel(((FDCAN_HandleTypeDef *)CANmodule->CANptr)) > 0) {
		...
		tx_hdr.TxFrameType =
			(buffer->ident & FLAG_RTR) ? FDCAN_REMOTE_FRAME : FDCAN_DATA_FRAME;
				      ...
				      uint32_t fifo_isrs)
{
	...
	rcvMsg.ident =
		rx_hdr.Identifier | (rx_hdr.RxFrameType == FDCAN_REMOTE_FRAME ? FLAG_RTR : 0x00);
...
void HAL_FDCAN_TxBufferCompleteCallback(FDCAN_HandleTypeDef *hfdcan, uint32_t BufferIndexes)
{
	...
	if (CANModule_local->CANtxCount > 0U) { /* Are there any new messages waiting to be send */
		CO_CANtx_t *buffer =
			&CANModule_local->txArray[0]; /* Start with first buffer handle */
		uint16_t i;
		 ...
		 * FDCAN instance and send data, then no need to lock interrupts..)
		 */
		...
		for (i = CANModule_local->txSize; i > 0U; --i, ++buffer) {
			...
			if (buffer->bufferFull) {
				if (prv_send_can_message(CANModule_local, buffer)) {
					...
					break; // if we could not send the message, break out of the
					       // loop (the tx buffers are full)
				}
			}
		}
