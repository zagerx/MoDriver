...
#define BOARD_RATE      (1000000)
#define DESIRED_NODE_ID (21)
...
int canopen_app_init(void)
{
	...
	// 	return 1;
	// } else {
	// 	log_printf("Allocated 0x%xbytes for CANopen objects\n", heapMemoryUsed);
	// }
...
static int canopen_app_resetCommunication(void)
{
	...
	CO_CANsetConfigurationMode((void *)&hfdcan2);
	CO_CANmodule_disable(CO->CANmodule);

	/* initialize CANopen */
	err = CO_CANinit(CO, &hfdcan2,
			 1000); // Bitrate for STM32 microcontroller is being set in MXCube Settings
	if (err != CO_ERROR_NO) {
	...
	err = CO_CANopenInitPDO(CO, CO->em, OD, DESIRED_NODE_ID, &errInfo);
	if (err != CO_ERROR_NO && err != CO_ERROR_NODE_ID_UNCONFIGURED_LSS) {
		if (err == CO_ERROR_OD_PARAMETERS) {
			log_printf("Error: Object Dictionary entry 0x%" PRIx32 "\n", errInfo);
		} else {
	...
	HAL_TIM_Base_Start_IT(&htim6); // 1ms interrupt
...
void canopen_app_process(void)
{
	...
	if ((time_current - time_old) > 0) { // Make sure more than 1ms elapsed
		...
		if (reset_status == CO_RESET_COMM) {
			...
			canopen_app_init(); // Reset Communication routine
