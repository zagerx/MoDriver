...
#define XXX_FLASH_PAGE_SIZE  2048U                                    /* 2KB per page */
#define FLASH_TOTAL_SIZE     (256U * 1024U)                           /* 256KB total */
#define FLASH_PAGE_NUM_TOTAL (FLASH_TOTAL_SIZE / XXX_FLASH_PAGE_SIZE) /* 128 pages */

/* 用户数据区配置 - 使用最后两页 */
#define FLASH_USER_PAGE_COUNT 2                                              /* 使用 2 页 */
#define FLASH_USER_START_PAGE (FLASH_PAGE_NUM_TOTAL - FLASH_USER_PAGE_COUNT) /* Page 126 */
#define FLASH_USER_START_ADDR (FLASH_BASE_ADDR + (FLASH_USER_START_PAGE * XXX_FLASH_PAGE_SIZE))
#define FLASH_USER_SIZE       (FLASH_USER_PAGE_COUNT * XXX_FLASH_PAGE_SIZE) /* 4096 bytes */
...
static uint32_t flash_get_page(uint32_t addr)
{
	return (addr - FLASH_BASE_ADDR) / XXX_FLASH_PAGE_SIZE;
}
...
static flash_status_t flash_erase_page(uint32_t page_addr)
{
	...
	/* 验证擦除结果 */
	for (uint32_t i = 0; i < XXX_FLASH_PAGE_SIZE; i += 4) {
...
void flash_write(uint32_t addr, uint8_t *data, uint16_t *datalen)
{
	...
	for (uint32_t page = 0; page < FLASH_USER_PAGE_COUNT; page++) {
		uint32_t page_addr = FLASH_USER_START_ADDR + (page * XXX_FLASH_PAGE_SIZE);
...
uint8_t flash_erase_user_page(uint8_t page_offset)
{
	...
	uint32_t page_addr = FLASH_USER_START_ADDR + (page_offset * XXX_FLASH_PAGE_SIZE);
...
{
	flash_write(FLASH_USER_START_ADDR, data, datalen);
}

void hardware_flash_read_params(uint8_t *data, uint16_t *datalen)
{
	flash_read(FLASH_USER_START_ADDR, data, datalen);
}
/**
